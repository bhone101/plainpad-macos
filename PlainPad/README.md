...
The build script produces `build/PlainPad.app` inside the app directory.
...
From the `PlainPad` app directory:
...
With full Xcode installed, open `PlainPad.xcodeproj`, select the shared **PlainPad** scheme, and choose Run. The project includes an application target and an XCTest target, with local ad-hoc signing configured. No team is required. From Terminal:
...
See `VERIFICATION.md` for what actually ran in this environment and the remaining manual checks.
